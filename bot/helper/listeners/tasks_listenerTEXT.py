...
class MirrorLeechListener:
    ...
    def __setModeEng(self):
        mode = f" #{'Telegram' if self.isLeech else 'Gandakan' if self.isClone else 'Rclone' if self.upPath not in ['gd', 'ddl'] else 'DDL' if self.upPath != 'gd' else 'GDrive'}"
        mode += ' (Zip)' if self.compress else ' (Unzip)' if self.extract else ''
        mode += f" Dari #{'Torent' if self.isQbit else 'Youtube' if self.isYtdlp else 'GDrive' if (self.isClone or self.isGdrive) else 'Mega' if self.isMega else 'Aria2' if self.source_url and self.source_url != self.message.link else 'Telegram'}"
    ...
    async def onDownloadComplete(self):
        ...
        async with download_dict_lock:
            ...
            gid = download.gid()
        LOGGER.info(f"Unduhan Selesai..: {name}")
        if multi_links:
