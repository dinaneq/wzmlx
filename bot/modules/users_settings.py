...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        thumbmsg = "Exists" if await aiopath.exists(thumbpath) else "Not Exist"
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        handler_dict[user_id] = False
        default_key = "ikT3TJMacPF8NNXJQ7G7G9z244hV0eQK"
        ddl_dict = user_dict.get('ddl_servers', {})
        key = data[2][1:] if len(data) > 2 else default_key
        mode, api = ddl_dict.get(key, [False, ""])
        if key not in ddl_dict:
          ddl_dict[key] = [False, ""]
        if data[2][0] == 's':
            if not mode and api == '':
