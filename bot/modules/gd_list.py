...
async def list_buttons(user_id, isRecursive=True):
    buttons = ButtonMaker()
    buttons.ibutton("Hanya Folder", f"list_types {user_id} folders {isRecursive}")
    buttons.ibutton("Hanya File", f"list_types {user_id} files {isRecursive}")
    buttons.ibutton("Keduanya", f"list_types {user_id} both {isRecursive}")
    buttons.ibutton(f"{'✅️' if isRecursive else ''} Recursive", f"list_types {user_id} rec {isRecursive}")
    buttons.ibutton("Batal", f"list_types {user_id} cancel")
    return buttons.build_menu(2)


async def _list_drive(key, message, user_id, item_type, isRecursive):
    LOGGER.info(f"Daftar GDrive: {key}")
...
async def select_type(_, query):
    ...
    if user_id != int(data[1]):
        ...
        return await editMessage(message, '<b>Pilih opsi daftar drive:</b>', buttons)
    elif data[2] == 'cancel':
        await query.answer()
        return await editMessage(message, "<b>Daftar telah dibatalkan!</b>")
    await query.answer()
    item_type = data[2]
...
async def drive_list(_, message):
    ...
    if len(args) == 1:
        return await sendMessage(message, '<i>Kirim kunci pencarian beserta perintah</i>')
    ...
    await sendMessage(message, '<b>Pilih opsi daftar drive:</b>', buttons, 'IMAGES')
